...
3. **If phpstan fails — fix type errors**
   phpstan scans `src/` only at level max with this relaxation in `phpstan.neon`:
   - `treatPhpDocTypesAsCertain: false`
   - Six optional-dependency handlers are excluded via `excludePaths`: `DoctrineDBALSessionHandler`, `DoctrineSessionHandler`, `IlluminateSessionHandler`, `InfluxDbSessionHandler`, `PhpCacheSessionHandler`, `PhpFastCacheSessionHandler`
